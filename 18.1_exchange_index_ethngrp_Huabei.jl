## ------------------------------------------------------------------------
##
## Script name: 18.1_exchange_index_ethngrp_Huabei.jl
...
birthy_vector = sort(unique(sample_Huabei[!, :birthy]))
...
for i in birthy_vector
    # Select sample_Huabeis within 11-year moving cohorts
...
birthy_vector = sort(unique(sample_Huabei[!, :birthy_sp]))
...
# Join to the sample_Huabei
sample_EI = @chain sample_Huabei begin
    @select(
...
ethngrp_vector = ["Hui", "Manchu", "Mongolian"]
...
for i in ethngrp_vector
    # Retrieve sample_Huabei
    hus_df, wif_df = retrieve_sample(i, "wif")
    # Match sample_Huabeis
...
EI_df_Huabei = @transform(EI_df, :region = "North")
EI_df_short_Huabei = @transform(EI_df_short, :region = "North")