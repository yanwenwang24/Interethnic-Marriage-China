## ------------------------------------------------------------------------
##
## Purpose: Analyze status exchange in provinces WITHOUT preferential birth policy
## Author: Yanwen Wang
...
# Join policy information to sample and create subsample for provinces WITHOUT policy
census = @transform(census, :hm_policy = get.(Ref(hm_policy_dict), :province, missing))
sample = @transform(sample, :hm_policy = get.(Ref(hm_policy_dict), :province, missing))

sample_no_policy = @chain sample begin
    @subset(:hm_policy .== 0)
end
...
birthy_vector = sort(unique(sample_no_policy[!, :birthy]))
...
birthy_vector = sort(unique(sample_no_policy[!, :birthy_sp]))
...
# Join to the sample
sample_EI = @chain sample_no_policy begin
    @select(
